...
1. 给 `render_agents_docs.py` 增加 `--emit-scoped`。
2. 扩展 inspectors，返回候选作用域：`main_sources`、`test_sources`、`karate_features`、`webflux_handlers`。
3. 只把模板片段渲染到匹配作用域中。
4. dry-run 输出每个将要生成的文件，以及生成原因。
...
这个例子里，每次修改模板或 renderer，都能用固定 Spring Boot Maven fixture 验证不会把项目初始化结果弄坏。
